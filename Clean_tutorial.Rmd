---
title: "Tutorial for inferring dynamic lncRNA regulation in autism spectrum disorder" 
author: "\\
...
output:
    ...
    BiocStyle::pdf_document:
      toc: yes
vignette: >
    %\VignetteIndexEntry{Tutorial for inferring dynamic lncRNA regulation in autism spectrum disorder} 
...
For convenience, the full list of our prepared single-nucleus RNA sequencing data in the Clean paper can be obtained from [here](https://drive.google.com/file/d/1sdLk7AcZszXYdV47_EMEVVhf0p1E_vDo/view?usp=drive_link).

# Delineating dynamic lncRNA regulation in ASD 
The utility functions for delineating dynamic lncRNA regulation are collected in the source file: **Clean.R**. In this tutorial, we select three sequential causality methods (Clean, Granger, seqICP) to infer dynamic lncRNA regulation from ASD single-nucleus RNA sequencing data. The priori information of lncRNA targets has two cases: None (no priori information), LncTar (priori information of LncTar).
...
```
## Step 3: Causal inference using Clean, Granger, seqICP
```{r, eval=TRUE, include=TRUE}
################################################################################  Clean application ############################################################################
# ASD and Normal
...
# Conclusions
In this tutorial, we apply Clean, Granger and seqICP to show how to infer dynamic lncRNA regulation in ASD. Moreover, we also perform heterogeneity analysis of the identified lncRNA causal regulatory networks across diverse brain biological contexts (i.e. diagnosis, region, age, sex and cell type). Taken together, Clean provides a useful method to help delineate dynamic lncRNA regulation in ASD, and offers technical support for precision therapeutics tailored to dynamic causal regulation states.
